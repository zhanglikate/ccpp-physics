...
!      parameter (numaer=50)
      ...
      REAL(kind_phys) TOLF                 !bs convergence criterion on function values
      PARAMETER (TOLF = 1.E-09)

      REAL(kind_phys) TOLMIN                 !bs criterion whether superios convergence to
      PARAMETER (TOLMIN = 1.E-12) !bs a minimum of fmin has occurred

      REAL(kind_phys) TOLX                 !bs convergence criterion on delta_x
      PARAMETER (TOLX = 1.E-10)

      REAL(kind_phys) STPMX                !bs scaled maximum step length allowed
      PARAMETER (STPMX = 100.)

      REAL(kind_phys) c303, c302
...
! Geometric Constants:
      ...
      REAL(kind_phys) & ! PI (single precision 3.141593)
        pirs
      PARAMETER (pirs=3.14159265358979324)
!      REAL(kind_phys)     PIRS ! PI (single precision 3.141593)
...
! coarse                               
! log(sginin)                           
      REAL(kind_phys) xxlsgn
! log(sginia)                           
      REAL(kind_phys) xxlsga

      REAL(kind_phys) xxlsgc
! log(sginic )                          
! log(sginin ) ** 2                           
      REAL(kind_phys) l2sginin
! log(sginia ) ** 2                           
      REAL(kind_phys) l2sginia

      REAL(kind_phys) l2sginic
...
! special factor to compute mass transfer           
      REAL(kind_phys) dgvem_i
      PARAMETER (dgvem_i=0.03E-6) ! [ m ]                            
      REAL(kind_phys) sgem_i
      PARAMETER (sgem_i=1.7)

! *** Accumulation mode:
      REAL(kind_phys) dgvem_j
      PARAMETER (dgvem_j=0.3E-6) ! [ m ]                             
      REAL(kind_phys) sgem_j
      PARAMETER (sgem_j=2.0)

! *** Coarse mode
      REAL(kind_phys) dgvem_c
      PARAMETER (dgvem_c=6.0E-6) ! [ m ] <<< Corrected 11/19/97      
      REAL(kind_phys) sgem_c
      PARAMETER (sgem_c=2.2)

! *** factors for getting number emissions rate from mass emissions rate
! Aitken mode                                       
      REAL(kind_phys) factnumn
! accumulation mode                                 
      REAL(kind_phys) factnuma

      REAL(kind_phys) factnumc
! coarse mode                                       
      REAL(kind_phys) facatkn_min, facacc_min
      PARAMETER (facatkn_min=0.04,facacc_min=1.0-facatkn_min)
      REAL(kind_phys) xxm3
      REAL(kind_phys), PARAMETER ::  conmin = 1.E-16
      REAL(kind_phys), PARAMETER ::  epsilc = 1.E-16
! [ ug/m**3 ] ! changed 1/6/98 
      REAL(kind_phys) & ! factor to set minimum for Aitken mode number  
        nummin_i
      REAL(kind_phys) & ! factor to set minimum for accumulation mode nu
        nummin_j
      REAL(kind_phys) & 
        nummin_c
! factor to set minimum for coarse mode number  
!bs
!bs      REAL(kind_phys) ALPHSULF ! Accommodation coefficient for sulfuric acid
!bs      PARAMETER ( ALPHSULF = 0.05 ) ! my be set to one in future
!bs
!bs      REAL(kind_phys) DIFFSULF ! molecular diffusivity for sulfuric acid [ m**2
...
! set to a value below                  
      REAL(kind_phys) ccofm
! minimum aerosol sulfate concentration          
      REAL(kind_phys) aeroconcmin
...
!   molecular weights (g/mol)
	REAL(kind_phys), parameter :: mw_so4_aer   = 96.066
	REAL(kind_phys), parameter :: mw_no3_aer   = 62.007
	REAL(kind_phys), parameter :: mw_nh4_aer   = 18.042
	REAL(kind_phys), parameter :: mw_oc_aer    = 250.0
	REAL(kind_phys), parameter :: mw_ec_aer    = 1.0
	REAL(kind_phys), parameter :: mw_oin_aer   = 1.0
	REAL(kind_phys), parameter :: mw_dust_aer  = 100.087
	REAL(kind_phys), parameter :: mw_seas_aer  = 58.440
	REAL(kind_phys), parameter :: mw_cl_aer    = 35.450
	REAL(kind_phys), parameter :: mw_na_aer    = 22.990
	REAL(kind_phys), parameter :: mw_water_aer = 18.016

!   dry densities (g/cm3)
	REAL(kind_phys), parameter :: dens_so4_aer  = 1.80   ! = rhoso4
	REAL(kind_phys), parameter :: dens_no3_aer  = 1.80   ! = rhono3
	REAL(kind_phys), parameter :: dens_nh4_aer  = 1.80   ! = rhonh4
	REAL(kind_phys), parameter :: dens_oc_aer   = 1.5    ! = rhoorg ! changed from 1.0
	REAL(kind_phys), parameter :: dens_ec_aer   = 1.70
	REAL(kind_phys), parameter :: dens_dust_aer = 2.60  ! = rhosoil
	REAL(kind_phys), parameter :: dens_oin_aer  = 2.20  ! = rhoanth
	REAL(kind_phys), parameter :: dens_seas_aer = 2.20  ! = rhoseas
	REAL(kind_phys), parameter :: dens_cl_aer   = 2.20
	REAL(kind_phys), parameter :: dens_na_aer   = 2.20

!   water density (g/cm3)
	REAL(kind_phys), parameter :: dens_water_aer  = 1.0

!   hygroscopicity (dimensionless)
	REAL(kind_phys), parameter :: hygro_so4_aer  = 0.5
	REAL(kind_phys), parameter :: hygro_no3_aer  = 0.5
	REAL(kind_phys), parameter :: hygro_nh4_aer  = 0.5
	REAL(kind_phys), parameter :: hygro_oc_aer   = 0.14
	REAL(kind_phys), parameter :: hygro_ec_aer   = 1.e-6
	REAL(kind_phys), parameter :: hygro_oin_aer  = 0.14
	REAL(kind_phys), parameter :: hygro_dust_aer = 0.1
	REAL(kind_phys), parameter :: hygro_seas_aer = 1.16
	REAL(kind_phys), parameter :: hygro_cl_aer   = 1.16
	REAL(kind_phys), parameter :: hygro_na_aer   = 1.16
...
! table lookup of aerosol impaction/interception scavenging rates
	...
	parameter (nimptblgrow_mind=-14, nimptblgrow_maxd=24)
     	REAL(kind_phys) scavimptblnum(4, nimptblgrow_mind:nimptblgrow_maxd, maxd_asize, maxd_atype), &
