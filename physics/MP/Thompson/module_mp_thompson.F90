...
! lzhang
      ...
      LOGICAL:: debug_flag
      INTEGER:: nu_c
      real:: aerosol_evaporation_factor,evap_term
!+---+
...
!+---+-----------------------------------------------------------------+ !  EVAPORATION
           elseif (clap .lt. -eps .AND. ssatw(k).lt.-1.E-6 .AND.     &
            ...
            Dc_star = DSQRT(Max(0.0,(-2.D0*DT * t1_evap/(2.*PI) &
...
!>  - Cloud water lookup table index.
 ...
 159         continue
            ...
            !prw_vcd(k) = MAX(DBLE(-rc(k)*0.99*orho*odt),                &
            !          -tpc_wev(idx_d, idx_c, idx_n)*orho*odt)
            !prw_vcd(k) = MAX(DBLE(-rc(k)*0.99*orho*odt), prw_vcd(k))
            !prw_vcd(k) = MAX(DBLE(-rc(k)*0.99*orho*odt), MIN(prw_vcd(k), -rc(k)*orho*odt))
 !lzhang

!            prw_vcd(k) = MAX(DBLE(-rc(k)*0.99*orho*odt), MIN(prw_vcd(k),&
!                    DBLE(-rc(k)*orho*odt - tpc_wev(idx_d, idx_c, idx_n)*orho*odt)))

             !prw_vcd(k) = MAX(DBLE(-rc(k)*0.99*orho*odt), MIN(prw_vcd(k),&
             !             DBLE(-rc(k)*orho*odt + evap_term*orho*odt)))
             ! crash: prw_vcd(k) = MAX(DBLE(-rc(k)*0.99*orho*odt), DBLE(-rc(k)*orho*odt)+evap_term*orho*odt)

             ! prw_vcd(k) = MAX(DBLE(-rc(k)*0.99*orho*odt), &
             !MIN(prw_vcd(k) + 0.2 * (DBLE(-rc(k)*orho*odt + evap_term*orho*odt) - prw_vcd(k)), &
             !        DBLE(-rc(k)*orho*odt + evap_term*orho*odt)))

             evap_term = MIN(tpc_wev(idx_d, idx_c, idx_n), rc(k)/odt) ! Limit evaporation to available cloud water rc(k)/odt
             prw_vcd(k) = MAX(DBLE(-rc(k)*0.99*orho*odt),                &
                      -evap_term*orho*odt)
